...
async function init(){
  ...
  /* Migration : l'ancien format stockait le planning directement par employé
     (un seul planning répété toutes les semaines). On le bascule dans la
     semaine en cours pour ne rien perdre. */
  const schedKeys = Object.keys(state.schedule);
  const isLegacy = schedKeys.length > 0 && !/^\d{4}-\d{2}-\d{2}$/.test(schedKeys[0]);
  if(isLegacy){
    const wKey = weekKeyForDate(new Date());
    state.schedule = { [wKey]: state.schedule };
...
function openScheduleEditor(empId, dayKey){
  const emp = state.employees.find(e=>e.id===empId);
  const sched = state.schedule[empId] || {};
  ...
  host.innerHTML = `
    <div class="card">
      ...
      <div class="field-row">
        <button class="btn" onclick="saveScheduleDay('${empId}','${dayKey}')">Enregistrer</button>
        <button class="btn secondary" onclick="document.getElementById('scheduleEditorHost').innerHTML=''">Annuler</button>
      </div>
    </div>
  `;
}
async function saveScheduleDay(empId, dayKey){
  ...
  state.schedule[empId][dayKey] = { repos, start, end, breakStart, breakEnd, posteId };
  await saveJSON('schedule', state.schedule);
...
function scheduledHoursForDate(empId, dateStr){
  const d = new Date(dateStr + 'T00:00:00');
  const dayKey = dayKeyForDate(d);
  const day = (state.schedule[empId] || {})[dayKey];
